...
public class Dashboard extends JFrame {
    ...
    private FormKontakDeveloper formKontakDeveloper;
    ...
    private void initComponents() {
        ...
        int badgeWidth = "Demo".equals(currentUser.getRole()) ? 52 : 
                        ("Admin".equals(currentUser.getRole()) ? 52 : 68);
        roleBadge.setBounds(70, 48, badgeWidth, 20);
    ...
    private JPanel createWelcomePanel() {
        ...
        if (currentUser.isDemo()) {
            ...
            demoBanner.setOpaque(false);
            demoBanner.setLayout(new BorderLayout(15, 0));
            demoBanner.setBorder(new EmptyBorder(18, 20, 18, 20));
            demoBanner.setMaximumSize(new Dimension(Integer.MAX_VALUE, 95));
            demoBanner.setAlignmentX(Component.LEFT_ALIGNMENT);
            
            JLabel iconDemo = new JLabel("🎭");
            iconDemo.setFont(new Font("Segoe UI Emoji", Font.PLAIN, 32));
            
            JLabel txtDemo = new JLabel("<html><b style='font-size: 15px;'>MODE DEMO AKTIF</b><br>" +
                "<span style='font-size: 11px;'>Anda dalam mode READ-ONLY. Semua fitur CRUD dinonaktifkan.<br>" +
                "Tertarik? Klik menu <b>'Hubungi Developer'</b> untuk info lebih lanjut!</span></html>");
            txtDemo.setForeground(Color.WHITE);
            
            demoBanner.add(iconDemo, BorderLayout.WEST);
            demoBanner.add(txtDemo, BorderLayout.CENTER);
            
            innerPanel.add(demoBanner);
            innerPanel.add(Box.createVerticalStrut(20));
        }
    ...
    /**
     * Disable semua tombol CRUD untuk demo user
     */
    ...
    /**
     * Helper method untuk disable buttons berdasarkan text
     */
    private void disableButtonsInPanel(JPanel panel, String... buttonTexts) {
        for (Component comp : panel.getComponents()) {
            disableButtonsRecursive(comp, buttonTexts);
        }
    }
    
    private void disableButtonsRecursive(Component comp, String... buttonTexts) {
        if (comp instanceof JButton) {
            ...
            for (String text : buttonTexts) {
                if (btnText != null && btnText.contains(text)) {
                    ...
                    btn.setToolTipText("🔒 Fitur ini dinonaktifkan dalam mode demo");
                    break;
                }
            }
        } else if (comp instanceof Container) {
            for (Component child : ((Container) comp).getComponents()) {
                disableButtonsRecursive(child, buttonTexts);
            }
        }
    }
}
